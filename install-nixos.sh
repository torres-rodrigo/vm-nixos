...
usage() {
  cat <<'USAGE'
Usage: install-nixos [--dry-run] [--repo-root PATH]
...
validate_repo_root() {
  ...
  [[ -f "$value/disko/disko.nix" ]] || die "repo root does not contain disko/disko.nix: $value"
...
# Do not modify this file! It was generated by install-nixos.sh.
{ lib, modulesPath, ... }:
...
import (builtins.toPath (${repo_root_nix} + "/disko/disko.nix")) {
  diskDevice = ${disk_nix};
...
main() {
  ...
  info ""
  info "Install complete for $host. Reboot, unlock LUKS with the shared password, then log in as user r."
