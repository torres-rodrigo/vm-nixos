...
The project is ready to leave this directory only when its flake evaluates,
builds, temporarily activates, persistently activates, survives reboot, and can
rebuild from its source; Mango provides a usable session; networking, audio,
graphics, portals, user access, and Home Manager integration work; secrets are
not committed; and no runtime or evaluation path depends on `.nixos/` or the
temporary `nixos/` nesting.
