...
WORK_DIR=""
LUKS_PASSWORD_FILE="/run/nixos-disko-luks-password"
SUPPORTED_HOSTS=(
  "war"
  "conquest"
)
...
usage() {
  cat <<'USAGE'
Usage: install-encrypted-nixos [--dry-run] [--repo-root PATH]

Install a selected NixOS host with a Disko-managed LUKS2 + Btrfs layout.
...
cleanup() {
  ...
  rm -f "$LUKS_PASSWORD_FILE" 2>/dev/null || true
}
...
prompt_username() {
  ...
  while true; do
    ...
    warn "username must start with a lowercase letter or underscore and contain only lowercase letters, digits, underscores, or hyphens"
  done
}
...
validate_repo_root() {
  ...
  [[ -f "$value/v0/disko/disko.nix" ]] || die "repo root does not contain v0/disko/disko.nix: $value"
...
is_supported_host() {
  ...
  for candidate in "${SUPPORTED_HOSTS[@]}"; do
...
select_host() {
  ...
  while true; do
    printf 'Select target host by number or name: ' >&2
    IFS= read -r selection

    if [[ "$selection" =~ ^[0-9]+$ ]] && (( selection >= 1 && selection <= ${#SUPPORTED_HOSTS[@]} )); then
      printf '%s' "${SUPPORTED_HOSTS[$((selection - 1))]}"
      return
    fi

    if is_supported_host "$selection"; then
      printf '%s' "$selection"
      return
    fi

    printf '%s\n' "Selection must be one of: ${SUPPORTED_HOSTS[*]}." >&2
  done
}
...
write_hardware_config() {
  ...
  case "$host" in
    war)
      write_war_hardware_config "$host_dir/hardware-configuration.nix" "$esp_uuid" "$luks_uuid" "$btrfs_uuid"
      ;;
    conquest)
      write_conquest_hardware_config "$host_dir/hardware-configuration.nix" "$esp_uuid" "$luks_uuid" "$btrfs_uuid"
      ;;
    *)
      die "unsupported host for hardware generation: $host"
      ;;
  esac
}

write_war_hardware_config() {
  local target=$1
  local esp_uuid=$2
  local luks_uuid=$3
  local btrfs_uuid=$4

  cat > "$target" <<HARDWARE_CONFIG
# Do not modify this file! It was generated by v0/install-nixos.sh.
{ lib, modulesPath, ... }:
...
write_conquest_hardware_config() {
  local target=$1
  local esp_uuid=$2
  local luks_uuid=$3
  local btrfs_uuid=$4
...
write_install_files() {
  ...
  password_file_nix=$(nix_string "$LUKS_PASSWORD_FILE")
  repo_root_nix=$(nix_string "$repo_root")
  host_nix=$(nix_string "$host")
  username_nix=$(nix_string "$username")
  user_home_nix=$(nix_string "/home/$username")

  cat > "$work_dir/install-overrides.nix" <<INSTALL_OVERRIDES
{ lib, ... }:
...
import (builtins.toPath (${repo_root_nix} + "/v0/disko/disko.nix")) {
  diskDevice = ${disk_nix};
  passwordFile = ${password_file_nix};
}
INSTALL_DISKO

  cat > "$work_dir/flake.nix" <<TEMP_FLAKE
{
  description = "Temporary encrypted NixOS installer wrapper";
...
Dry run complete. The installer would run:
  ...
  set networking.hostName to "$host"
  set initial root and "$username" password hashes from the shared bootstrap password
  nixos-install --no-write-lock-file --flake "$work_dir#$host" --no-root-passwd
...
main() {
  ...
  info "Selected username: $username"
  ...
  secret=$(prompt_secret "Shared LUKS/root/$username password")
  ...
  info ""
  important "DESTRUCTIVE ACTION: this will permanently erase all data on:"
  important "  $resolved_disk"
  warn "The disk will be repartitioned, encrypted, formatted, mounted, and installed as NixOS."
  ...
  write_install_files "$WORK_DIR" "$repo_root" "$resolved_disk" "$host" "$username"

  if [[ $dry_run -eq 1 ]]; then
    dry_run_commands "$WORK_DIR" "$repo_root" "$host" "$username"
    exit 0
  fi
  ...
  info ""
  success "Install complete for $host."
  info "Reboot, unlock LUKS with the shared password, then log in as user $username."
